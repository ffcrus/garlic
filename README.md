...
Если хотите, чтобы переключение сопровождалось звуком, необходимо раскомментировать строчку со звуком, чтобы она выглядела как `paplay $SCRIPT_DIR/notification.wav` (удалить решётку в самом начале).
